...
v2_file=0
get_latest_ver(){
    ss_file=$(wget -qO- https://api.github.com/repos/shadowsocks/shadowsocks-rust/releases/latest | grep name | grep -v sha256 | grep x86_64 | grep gnu.tar.xz | cut -f4 -d\")
    v2_file=$(wget -qO- https://api.github.com/repos/shadowsocks/v2ray-plugin/releases/latest | grep linux-amd64 | grep name | cut -f4 -d\")
}

# Set shadowsocks-rust config password
set_password(){
    echo "\033[1;34mPlease enter password for shadowsocks-rust:\033[0m"
...
install_libsodium(){
    if [ -f /usr/lib/libsodium.a ] || [ -f /usr/lib64/libsodium.a ];then
        ...
        if [ ! -f libsodium-$LIBSODIUM_VER.tar.gz ];then
            curl -L -o libsodium-$LIBSODIUM_VER.tar.gz https://download.libsodium.org/libsodium/releases/LATEST.tar.gz
        fi
...
install_ss(){
    if [ -f /usr/local/bin/ssserver ];then
        echo "\033[1;32mshadowsocks-rust already installed, skip.\033[0m"
    else
        if [ ! -f $ss_file ];then
            ss_url=$(wget -qO- https://api.github.com/repos/shadowsocks/shadowsocks-rust/releases/latest | grep -v sha256 | grep browser_download_url | grep x86_64 | grep gnu.tar.xz | cut -f4 -d\")
            wget $ss_url
        fi
        xz -d $ss_file
...
remove_all(){
    ...
    rm -f /usr/local/lib/libshadowsocks-rust.a
    rm -f /usr/local/lib/libshadowsocks-rust.la
    rm -f /usr/local/include/shadowsocks.h
    rm -f /usr/local/lib/pkgconfig/shadowsocks-rust.pc
    rm -f /usr/local/share/man/man1/sslocal.1
    rm -f /usr/local/share/man/man1/ssservice.1
    rm -f /usr/local/share/man/man1/ssserver.1
    rm -f /usr/local/share/man/man1/ssmanager.1
    rm -f /usr/local/share/man/man1/ssurl.1
    rm -f /usr/local/share/man/man8/shadowsocks-rust.8
    rm -fr /usr/local/share/doc/shadowsocks-rust
